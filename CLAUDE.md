...
- **Post-pull hints**: a `configs` pull — via `gpullall` *or* a `gpushall` rebase that integrated remote changes — that changed `bash/` files, added/removed files in a stow package, or touched `nvim/lazy-lock.json` prints the required follow-up (`source ~/.bashrc`, `stow-all`, `nvim --headless "+Lazy! restore" +qa`) — the silent-skip trap from "Daily Sync Workflow" above, plus the plugin-version skew described in the 2026-08-22 Neovim follow-up audit. The change list is parsed NUL-delimited, so non-ASCII filenames (which git C-quotes in plain output) can't silently defeat it. **Hints are consolidated (2026-08-22):** they are queued during the run and printed as one block after the summary, rather than interleaved through fourteen repos' output where they were easy to scroll past. The queue is `local` to each command, the flush sits before the final status expression so exit codes are unaffected, and only the *message* defers — `claude-link --auto` still runs where it arises. Known cost: a Ctrl-C mid-run loses the queued hints for good, since the next run finds the repo already up to date.
...
### Follow-up audit (2026-08-22, second pass)

A full-config audit on `bigfed` after the latency work, covering the LazyVim layer the first pass never reached. Verdicts recorded so they are not re-derived:
...
- **Auto-save failures are loud now, once per failure episode** — see the amended auto-save section above, including the `'confirm'`-dialog trap the fix itself uncovered.
...
**Failures are loud now — and the loudness needed a guard (2026-08-22).** `silent! write` swallowed a failed write entirely: modified buffer, nothing shown, even `v:errmsg` empty — so on a full disk (the bigfed ENOSPC class) the invariant "normal mode ⟹ saved" failed without a word, indefinitely. The write now runs under `pcall`: success is exactly as silent as before; the first failure per buffer raises one WARN with the E-code, repeats stay quiet, and a save succeeding again notes once at INFO. Readonly/nomodifiable buffers are skipped outright — outside the invariant, nothing to fight. The non-obvious part: LazyVim sets `'confirm'`, under which a failing `:write` does not error at all — it pops a **modal dialog** and blocks, which `silent!` had been suppressing along with everything else. So the callback drops `'confirm'` for the duration of the write, and failures raise catchable errors instead of prompts. `:wqa` remains the quit-time backstop (it refuses to quit over a failing write); the notification covers the mid-session window where editing would otherwise continue on a dead disk. Pinned by five checks in `tests/nvim-latency`, mutation-verified: deleting the confirm-guard brings the dialog back and fails exactly the disk-failure checks.

**Regression suite:** `tests/nvim-latency/run.sh` (37 checks, ~40 s, hermetic — self-contained fixture, edits on a `$TMPDIR` copy, persistence disarmed, no repo touched; needs `pynvim`). It asserts no milliseconds, because timings move with the machine and make a poor gate; it asserts the *structure* the speed-up rests on and the behaviour it must not have broken. Mutation-verified against a pre-fix tree via `NVIM_LATENCY_CONFIG`, where exactly the seven expected checks fail. `bench.py` beside it is the measurement tool (`--ablate`, `--file`, `--config`), and `stallwatch.lua` catches stalls in a live session. Read `tests/nvim-latency/README.md` before changing any of it — it records which checks were once vacuous and why.

### The live-server root wrapper (reworked 2026-08-22)

`lua/plugins/live-server.lua` starts/stops live-server.nvim (`<localleader>hh` / `<localleader>hk`) with the directory passed explicitly at both ends plus one remembered string (`last_root`), so the stop works from any buffer in any project. Bare start/stop resolve from the current buffer and — via a trailing-slash instance key the plugin's upward walk never reproduces — silently miss explicitly-started instances; the full record, the measured losses of simpler shapes, and the declined alternatives are in `TODO.md` item 9.

Root resolution is `project_root()`: the nearest `.git` upward from the *buffer*, then `git rev-parse` in *nvim's cwd*, then `getcwd()` — so in a git-less directory the cwd acts as the root. On top of that, one branch: a root with `index.html` is served as-is; a root without one (an ops-style repo with the site in a subdirectory — the planned `nousowl.net` → `nousowl/` layout) gets the *buffer's* directory served instead, deliberately: the Linux watch is non-recursive on the served root, so serving the repo root there would kill live-reload for everything below it. An unnamed buffer degrades to serving the cwd.

**Regression suite:** `tests/live-server/run.sh` (27 checks, ~10 s, hermetic — fixtures and state under `mktemp -d`, loopback only, `browser=false`, real repos untouched). W checks pin the wrapper contract, D checks the no-repo directory semantics above, U checks pin *upstream* behaviours as canaries — a U failure after a plugin update means the ground moved, not necessarily that anything broke; read the suite's README before "fixing" either side. Mutation-verified in three directions with disjoint fingerprints. Run it after editing the wrapper or after any live-server.nvim update.
...
Sway uses vim-style navigation (hjkl). Mod4 (Super) is the primary modifier. Waybar config is JSON + `style.css`. The binding grammar is stated at the top of `sway/config` itself — since 2026-08-22 an explicit law (the modifier names what the action acts on: you / the focused window / the session, with key-family overrides and a closed promotion list) — read that before adding a binding, and note that every motion is bound for **both** vim keys and arrow keys, deliberately. `sway/README.md` is a hand-maintained quick-reference table of the bindings (named `README` so stow ignores it) — a best-effort courtesy, not a contract: `config` is authoritative, the file may lag it or be deleted at will, and the desktop suite deliberately does not check the two against each other (drift check offered and declined 2026-08-22).
...
Note `$mod+b` is a launcher here (Brave), not upstream sway's horizontal split — and the split bindings themselves, along with `focus parent`/`focus child`, were dropped on 2026-08-22 when the grammar was made explicit: the container tier goes unused in this workflow, and the config's Layout section records the reasoning plus the revival chords (TODO.md item 6 has the full account). `$mod+v` was a VS Code launcher until 2026-08-09, when VS Code was uninstalled and the binding removed — the key is now **free**.
