...
|---|---|---|---|---|
| preamble | `french-logic-preamble.sty` | fontenc T1, inputenc utf8, lmodern, babel english; ragged2e, setspace, multicol, xcolor, tabularx, mdframed, calc, adjustbox, booktabs, bm, centernot; `\graphicspath` | those | nothing |
| core | `french-logic-core.sty` | symbol fonts (`\Yright`, `\shortminus`, `\strictif`); sets, tuples, powerset; the `f` functions; languages and logics (`\lang`, `\logic`); relations (`\proves`, `\models`, `\trues`, sequent arrows); modal structures (`\M`, `\F`, `\nec`, `\poss`, canonical decorations); truth sets, proof sets, valuations; the underlined metalanguage; closures and the closure arrows | amsmath, amssymb, stmaryrd, mathrsfs, xspace, soul (`\st` is a renewal of soul's), graphicx, trimclip, the txfonts `txsyc` glyph | nothing |
...
| io | `french-logic-io.sty` | `\IO` and the IO systems, in/out/deriv, the rule names `\TOP` … `\CT` and their starred forms | amsmath, xspace | nothing |
| stit | `french-logic-stit.sty` | `\cstit`, `\dstit`, the structure names via `\versal` | microtype | nothing |
| proof | `french-logic-proof.sty` | the Set-Set … framework names, the subscripted `\proves` variants, the `//` consequence separator `\infer` | amssymb, amsthm, microtype | core (`\proves`) |
| gentzen | `french-logic-gentzen.sty` | the `gentzen` proof-tree environment over ebproof, rule labels, the signed-formula `\af`/`\de` family | ebproof, relsize (not yet loaded: defect D4) | nothing |
...
- **Mode.** Names — systems, axiom schemata, frame conditions, rules, I/O
  ...
  fact: 295 members work in both modes, 189 in math only, 14 in text only.
  `\Lc` gained the wrapper its `\lang` siblings have on 2026-09-07.
...
- **Names.** Uppercase family names are axioms, lowercase their frame
  ...
  than use. Single letters: fraktur `\A \C \F \M` are structures, bold `\B
  \D \E \K \T` are systems, `\Slog`, `\four`, `\five` the forced exceptions.
  `\mathsf{{O}}` keeps double braces as the placeholder for a future bold
  switch. Three naming schemes coexist today: `\kax` (letters plus `ax`, 35
  names, 17 live) for the classical schemata, `\CMr` with lowercase `\cmr`
  (101 names, 74 live) for the conditional families, bare `\CP`, `\CTh`, and
  `\cdax`, `\cduax` for the rest; three rule names lack the `rule` suffix;
  `\FRo` gives superscript 2 where `\FNo` gives 1; `\topt` gives subscript A;
  `\Ts` is the starred form of `\TOP`; `\Lc` sits outside the `\lang` family;
  the `solo` rule differs between the conditional operators (drop the trailing
  *s*) and the metalanguage connectives (append).
- **A grammar for the names — proposed 2026-09-07, decision deferred to a
  later chat.** A name is the letters as printed, CamelCase, digits spelled
  out. Kind is carried by case and one suffix: an initial capital for a schema
  ...
  letter printed (`\topa`, `\MR`, `\langc`). *Strict* applies all of it, and
  the classical schemata gain their capital while the order conditions lose
  theirs; *lenient* keeps `\kax` and `\Rup` as recorded exceptions and renames
  only the outliers (`\rmposs` → `\rmpossrule` and kin, `\Ts` → `\TOPs`,
  the frame numbering, `\topt`, `\bott`, `\MRel`, `\Lc`, `\SDLt`). Every
  rename with its live-use counts, and the tool consequences (the highlight
  patterns `axfam`, `orderfam`, `rpossfam`; the snippets; one substitution
  pass over the dissertation), are on the type board, third sitting.
...
- **Provenance.** `\Yright` as the sequent separator (`\seq`, `\tseq`) is
  ...
  redundancy.
- **Duplicates.** Eleven groups of byte-identical definitions are deliberate
  vocabulary (`\then`, `\onlyif`, `\to`; `\gives`, `\asserts`, `\proves`;
  the four names for `\Rightarrow`; …) until decided otherwise — principle 1.
...
- **Highlighting.** `nvim/lua/plugins/vimtex.lua` registers members into the
  register taxonomy by name shape (`C[CM][rl]\w*`, `[ckdtupwn]ax\w*`,
  `\w*rule\w*`, …); renames must keep to those shapes or update them. The
  coverage check is `sty-lua-snippets.py --coverage`.
- **Rendering suite.** `tests/french-logic/run.sh`; golden re-accepted after
  each landed change, last on 2026-09-07 after the decorations and folds; the
  split changed no page. `extras.tex` adds fixture pages for what has no
  ...
  unit files; `tests/french-logic/units.sh` is the standalone-load test.
...
was closed. Four opuscula
documents fail against the current package because they define macros the
package has since acquired (`\IO`, `\lang`, `\SDLt`, `\suchthat`) or use the
retired `defn` environment; that is consumer drift, not a package defect,
...
- **D6** `\IOfn` set bare `\tiny` in a superscript; seven warnings per
  ...
  2026-09-07: `\text{\tiny\textsf{H}}`, the size the definition asked for;
  the IO-logic chapter's 23 sites now match `\IOhn`'s label height.*
...
- **F1** mode, face, and naming drift as listed under *Conventions*. *Mode,
  face, labels, decorations, and structure indices settled 2026-09-07; the
  naming grammar is proposed and awaits a decision; the per-unit ordering
  passes remain.*
...
2026-09-07 and left as it is. Pending: the naming grammar; the per-unit
ordering passes; the dissertation charter's package paragraph, which still
describes aliascnt, the old `deon` and an absolute path.
