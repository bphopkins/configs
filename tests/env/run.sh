...
#
# Caged: the suite re-executes itself inside a systemd user scope with a
# memory ceiling, swap off, a task cap and a time cap, and stops that scope on
# the way out (org/claude-config/rules/system-and-server-work.md). "In a cage"
# is read from the cgroup's own memory.max, never from a variable. No user
# manager, no run.
#
...
if ! caged; then
  if ! command -v systemd-run >/dev/null 2>&1 ||
    ...
    echo "env tests: no user manager to cage the run in -- not running" >&2
    echo "passed: 0  failed: 1"
    exit 2
  fi
  avail_kb="$(awk '/^MemAvailable:/ {print $2}' /proc/meminfo)"
  if ((avail_kb < (512 + 1024) * 1024)); then
    echo "env tests: less than 1.5G available; the 512M cage plus a 1G floor does not fit -- not running" >&2
    echo "passed: 0  failed: 1"
    exit 2
  fi
  unit="env-tests-$$-$RANDOM"
