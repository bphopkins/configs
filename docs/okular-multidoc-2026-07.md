...
*Annotated 2026-09-23.* The condition was met — PDFs are opened from the desktop as well as from VimTeX (confirmed 2026-09-21) — and closed without the drop-in: `bash/.bash_profile` now sources `10-env.sh` and `20-path.sh`, so the login shell GDM starts carries `~/bin` and `~/.local/bin` (and TeX Live, in `20-path.sh`'s own order) into the session, the way Fedora's unguarded skeleton `.bashrc` does on a stock account. The reason above survives: PATH keeps one author. `DECISIONS.md`, 2026-09-23; `org/machines/environment.md`.
