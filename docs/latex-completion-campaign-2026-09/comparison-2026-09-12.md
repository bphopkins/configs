...
- Loading per document: three always-on lists (tex, latex-document,
  ...
  Annotation, 2026-09-13 (build chat 2, from `src/latexstyleparser.cpp`):
  every autogenerated row carries `#S`, hidden from the completer and kept
  for the syntax checker, so the bare list completes nothing; the
  `#include` lines are the part the campaign borrows. Its other rules are
  not the specification for the `.sty` mode: a command with a non-empty
  optional default yields both signatures, an empty default `[2][]` is read
  as no optional argument at all, `\newenvironment` yields a bare
  `\begin{name}`, `\DeclareMathSymbol` is marked math, and a
  `\RequirePackage[` whose bracket closes on the next line is missed.
...
Annotation, 2026-09-13 (build chat 1): the generator built from these
rules reproduces the spike's per-file counts of typical commands exactly
for amsmath, biblatex, class-memoir, tikz and tex. The spike's filter
drops any row with a `*` anywhere in its classification, which also drops
five latex-dev rows classified `#beginEnv*#%expl3`, where the `*` is not
the unusual marker; TeXstudio shows them as typical, and so does the
generator. The counts above are otherwise confirmed.
...
| keystroke cost at scale (bench method, fixture's 2,079-char line, fedxps balanced) | today, 1,054 snippets: command-name median 146 ms, p90 190; prose 92 ms. With 9,054 rows: command-name 160 ms, p90 192; prose 94 ms. Source called once per command word (5 to 20 ms per request), fuzzy about 2 to 8 ms per keystroke; transform 7.2 ms cold, 2.5 ms warm; blink's per-request copy 4.8 ms |
